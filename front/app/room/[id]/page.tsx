...
const Room = ({ params }: { params: { id: string } }) => {
  ...
  const socketRef = useSocket("zoom");
  ...
  const handleRoomJoined = () => {
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        socketRef.current.emit(E.READY, roomName);
      })
      .catch((err) => {
        /* handle the error */
    ...
    if (hostRef.current) {
      ...
      rtcConnectionRef.current
        ...
        .then((offer) => {
          ...
          socketRef.current.emit(E.OFFER, { offer, roomName });
        })
        .catch((error) => {
          console.log(error);
        });
    ...
    if (!hostRef.current) {
      ...
      rtcConnectionRef.current
        ...
        .then((answer) => {
          ...
          socketRef.current.emit(E.ANSWER, { answer, roomName });
        })
        .catch((error) => {
          console.log(error);
        });
  ...
  const leaveRoom = () => {
    socketRef.current.emit(E.LEAVE, roomName); // Let's the server know that user has left the room.
